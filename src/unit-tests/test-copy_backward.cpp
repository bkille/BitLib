...
TYPED_TEST(DoubleRangeTest, CopyBackward) {
    for (size_t idx = 0; idx < this->bit_size - 1; ++idx) {
        ...
        long long n = bitvec1.size() - start1 - end1;
        auto bitret = bit::copy_backward(bitvec1.begin() + start1, bitvec1.end() - end1, bitvec2.begin() + start2 + n); 
        auto boolret = std::copy_backward(boolvec1.begin() + start1, boolvec1.end() - end1, boolvec2.begin() + start2 + n); 
