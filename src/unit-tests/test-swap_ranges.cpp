...
TYPED_TEST(DoubleRangeTest, SwapRanges) {
    for (size_t idx = 0; idx < this->bit_size - 1; ++idx) {
        ...
        unsigned long long start1 = generate_random_number(0, std::min<unsigned long long>(bitvec1.size() - 1, digits + 1));
        unsigned long long start2 = generate_random_number(0, std::min<unsigned long long>(bitvec2.size() - 1, digits + 1));
